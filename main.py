...
async def on_message(message):
    ...
    if '!fifa' == (str(message.content).lower()):
        user_ids = [701955314161025086, 321389614940028929, 1028533279802011688, 406686196581007361, 358470646549839874, 330886381075169284, 
                    133033000114847744, 373560731196456980, 182935000046370816, 240929868957745155, 
                    183630868114309120, 797942677844131911, 284441097545973762]

        mentions = " ".join([f"<@{user_id}>" for user_id in user_ids])

        await message.channel.send(f'{mentions}')
        await message.channel.send(file=discord.File(f'static/fifa1.gif'))
...
async def cosmo(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title = 'Lançamentos!!',
        description=f'Todos os lançamentos de hoje\n',
        color = 0xccff66
    )
    ...
    embed.add_field(name = f'Hoje temos:', value = results)
...
async def cosmo(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title = 'Lançamentos!!',
        description = f'Todos os lançamentos dessa proxima semana\n',
        color = 0xccff66
    )
    ...
    for dia, eventos in results.items():
        eventos_str = "\n".join(eventos) if eventos else "Sem lançamentos nessa data"
