...
class QuackBet(commands.Cog):
    ...
    @app_commands.command(description='Mostra o ranking de pontos do Quack Bet')
    async def ranking_pontos(self, interaction: discord.Interaction):
        await interaction.response.defer()

        usuarios_db = quack_banco.get_ranking()
        ...
        for idx, (user_id, pontos) in enumerate(usuarios_db, start=1):
            ...
            linha = (
                f"{idx:>2}º {nome:<20}"
                f"{float(pontos):<6.2f} Pontos {medalha}\n"
            )

            if idx <= top_ranking_numero:
                top_ranking += linha
            elif idx > top_ranking_numero and idx <=10:
        ...
        embed = discord.Embed(
            title="Quack Bet • Ranking Geral 🦆",
            description=descricao,
            color=get_sort_triples_color()
        )

        embed.set_footer(text="Quack Bet • Ranking Geral 🦆")
